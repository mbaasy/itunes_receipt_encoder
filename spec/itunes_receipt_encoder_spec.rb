...
describe ItunesReceiptEncoder do
  ...
  describe '#to_unified' do
    subject do
      described_class.new(attrs).to_unified
    end

    context 'decoding' do
      subject { ItunesReceiptDecoder.new(super()).receipt }
      ...
      it 'sets receipt properties' do
        ...
        expect(subject[:creation_date]).to eq(creation_date.strftime('%FT%TZ'))
  ...
  describe '#to_transaction' do
    subject do
      described_class.new(attrs).to_transaction(options)
    end
